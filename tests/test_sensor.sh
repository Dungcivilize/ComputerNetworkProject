...
for i in $(seq 1 $COUNT); do
    ...
    if [ "$BACKGROUND" == "true" ]; then
        ...
        gnome-terminal --title="TestSensor ns${i}testsensor ($ipaddr:$sensor_port)" -- bash -c "ip netns exec $ns $ROOT_DIR/tests/test_sensor $sensor_port sensor_${i} name_${i} password_${i}; exec bash"
    fi
done
