...
#include "handler/water.hpp"
#include "handler/schedule.hpp"

int main(int argc, char* argv[])
{
    cout << "Sprinkler running:\nID: " + id + "\nPassword: " + password << endl;
    ...
    /*
    pthread_t background2;
    if (pthread_create(&background2, nullptr, work2_thread, NULL) != 0)
    {
        perror("pthread_create");
        close(server_fd);
        return 1;
        ...
        for (size_t i = 1; i < pfds.size();)
        {
            ...
            if (revents & POLLIN)
            {
                ...
                if (cmd == "SCAN")
                {
                    ...
                    handle_water(content, reply);
                else if (cmd == "SCHEDULE")
                    handle_schedule(content, reply);
                else
